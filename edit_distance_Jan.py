...
def levenshteinDistance(target, source):
    n = len(target)
    m = len(source)
        ...
        for j in range(1, m+1):
            if target[i-1]==source[j-1]:
                ...
                distance[i][j] = min((distance[i-1][j] + in_cost(target[i-1]),       # insertion von target_i in source
                                      distance[i-1][j-1] + sub_cost(target[i-1], source[j-1]),     # substituition von target in source
...
print(levenshteinDistance("Hand","H/a/n/d/b/a/l/l"))