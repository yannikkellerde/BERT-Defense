...
cheap_actions = an array d with a true or false statment for each action
                          ...
                          d[2] == True: activate cheap delition
                          d[3] == True: activate cheap transposition
                          shortcut: True activates all and False deactivated all
"""
def levenshteinDistance(target, source, cheap_actions=False , word_embedding=None, char_app=None,vowls_in=None,cheap_deletions=True):
    ...
    for num in range(1, m+1):
        j += del_cost(source[num-1],char_app,False,freelo_amount) if cheap_actions[2] else 1
        distance[0][num] = j
    for i in range(1, n+1):
        for j in range(1, m+1):
            possibilities = [distance[i-1][j] + (in_cost(target[i-1], vowls_in) if cheap_actions[0] else 1),# insertion von target_i in source
                                ...
                                distance[i][j-1] + (del_cost(source[j-1], char_app,i==n and cheap_deletions,freelo_amount) if cheap_actions[2] else 1)]# delition  source_j
...
def get_word_dic_distance(word, dic, word_embedding, cheap_actions=False, sort=True, progress=True, orig_word=None, cheap_deletions=True):
    ...
    if len(word)>20:  # Filter out links and other uncomprehensable stuff
        ...
        for sample_word in (tqdm(dic) if progress else dic):
            distance.append((sample_word, *levenshteinDistance(sample_word, word,cheap_actions=cheap_actions,
                                                               ...
                                                               cheap_deletions=cheap_deletions)))
    if sort:
...
if __name__ == '__main__':
    ...
    distance = get_word_dic_distance("lohle", dic, word_embedding,cheap_actions=True, cheap_deletions=False)
    for i in range(40):
        print(distance[i])